...
**2. Cuatro servidores, tres lenguajes, dos transportes.**

```
brewops     stdio  Go          9 tools
filesystem  stdio  TypeScript  14 tools
git         stdio  Python      12 tools
netprobe    http   Go/Docker   3 tools
                               38 tools
```
...
**Interfaz de terminal.** Registro de frames en vivo con resúmenes de una línea,
progreso de herramientas mientras se ejecutan, y comandos `/tools`, `/log`,
`/usage`, `/reset`.
...
### 2.1 El servidor que se colgaba sin motivo aparente
...
### 2.3 La firma que había que devolver
...
### 2.4 Dos cuotas distintas disfrazadas del mismo error
...
### 2.5 El contenedor que construía bien y fallaba al arrancar
...
### 2.6 El límite invisible del lector
...
Tres de los cuatro servidores no usan la pila de red en absoluto:
...
Este proyecto conecta **cuatro servidores, escritos en tres lenguajes, por dos
organizaciones distintas, sobre dos transportes distintos**, y le entrega sus 38
